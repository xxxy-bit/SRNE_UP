...
class BmsLayout(QWidget):
    ...
    def initUI(self):
        ...
        self.tabWidget.addTab(self.tab_monitor,'实时监控')
        self.tabWidget.addTab(self.pal_monitor,'并机监控')
        self.tabWidget.addTab(self.tab_setParams,'参数设置')
        self.tabWidget.addTab(self.sys_seting,'系统设置')
        self.tabWidget.addTab(self.tab_hisdata,'历史数据')
        # self.tabWidget.addTab(self.tab_data,'实时数据')   # 输入密码后才可显示
        self.tab_monitorUI()
        self.pal_monitorUI()
        self.tab_dataUI()
    ...
    def pal_monitorUI(self):
        ...
        ed = [
            QLabel('平均电压(V)：'), self.avg_voltage ,QLabel('剩余容量(AH)：'), self.sur_cap,
            QLabel('平均SOC(%)：'), self.avg_soc ,QLabel('充满容量(AH)：'), self.full_cap,
            QLabel('总电流(V)：'), self.total_elc ,QLabel('SOH(AH)：'), self.soh,
            '','',QLabel('循环次数：'), self.loop_num
        ]
        ...
        self.col_labels = [
            ...
            '温度1','温度2','温度3','温度4','温度5',
            '温度6','温度7','温度8','温度9',
            'PACK电流','PACK总电流','PACK剩余容量','PACK满充容量',
            ...
            'PACK充电电流告警','PACK总电压告警','PACK放电电流告警','保护状态1',
            '保护状态2','指示状态','控制状态','故障状态',
            '均衡状态1','均衡状态2','告警状态1','告警状态2',
        ]
    ...
    def tab_monitorUI(self):
        ...
        battery_groupBox = QGroupBox("电池信息")
        ...
        temperature_groupBox = QGroupBox("温度信息")
        tem_h = QHBoxLayout()
        ...
        for i in range(8):
            self.cell_temp_8[f'cell温度_{i+1}'] = QLineEdit()
        ...
        for i in range(8, 16):
            self.cell_temp_16[f'cell温度_{i+1}'] = QLineEdit()
        ...
        self.tem_other = {
            '环境温度': QLineEdit(),
            'Pack正极端子温度': QLineEdit(),
            'Pack负极端子温度': QLineEdit(),
            'Pack端子温度3': QLineEdit(),
            'Pack端子温度4': QLineEdit(),
            'MOS管1温度': QLineEdit(),
            'MOS管2温度': QLineEdit()
        ...
        voltage_groupBox = QGroupBox("单体电压(V)")
        vol_H = QHBoxLayout()
        ...
        for i in range(8):
            self.cell_vol_8[f'cell电压_{i+1}'] = QLineEdit()
        ...
        for i in range(8, 16):
            self.cell_vol_16[f'cell电压_{i+1}'] = QLineEdit()
        ...
        self.getP01_data_btn = QPushButton('开始监控')
        self.getP01_data_btn.setStyleSheet(color_close)
        wd = [
            QLabel('串口:'), self.port_combobox, QLabel('波特率:'), self.baud_combobox, self.open_port_btn,
            QLabel('Pack:'), self.pack_combobox, QLabel('Pack数量:'), self.pack_num_line, self.getP01_data_btn,
            QLabel('地址:'), self.address_line, QLabel('间隔（秒）:'), self.space_combobox, ''
        ]
        ...
        sysStatus_groupBox = QGroupBox('系统状态')
        ...
        warnStatus_groupBox = QGroupBox('故障状态')
        ...
        protectStatus_groupBox = QGroupBox('保护状态')
        ...
        errorStatus_groupBox = QGroupBox('告警状态')
        ...
        openStatus_groupBox = QGroupBox('开关控制')
        ...
        login_groupBox = QGroupBox('管理员登录')
        login_H = QHBoxLayout()
